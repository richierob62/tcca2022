...
import type { ExpandedReceipt } from '../routes/reconciliation/receipts'
...
const ReceiptListForClerk: React.FC<Props> = ({ receipts }) => {
  ...
  const Cell = ({ index, style }: { index: number; style: any }) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ width: 200 }} className="text-left ">
          {receipts[index].client.name}
        </div>

        <div style={{ width: 150 }} className="text-left ">
          {receipts[index].receivedBy.name}
        </div>

        <Link
          to={`/loans/${
            receipts[index].loan.id
  ...
  const totalWidth = 48 + 80 + 100 + 100 + 200 + 150 + 150
  ...
  return (
    <div className="relative overflow-y-scroll w-max">
      <div className="sticky top-0 z-10 flex px-6 py-3 text-xs font-medium uppercase text-slate-700 bg-gray-50 drop-shadow-lg">
        ...
        <div className="text-left" style={{ width: 150 }}>
          Recvd By
        </div>
...
export default ReceiptListForClerk
